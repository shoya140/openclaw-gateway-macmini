...
- Mac Mini 上で Ollama を起動し `qwen3.6:35b-a3b` を `ollama pull` 済みであること（personal-agent 用。別モデルにする場合は `agents.list[]` を編集）
...
対話で **2 つの Telegram Bot Token (official / personal)** / User ID / Anthropic API Key を入力する。
...
- `~/.openclaw/openclaw.json` 生成（主な設定）:
  - **マルチアカウント Telegram**: `channels.telegram.accounts.{official,personal}` (各 `tokenFile` 指定) + `defaultAccount: official`。チャンネル直下の `allowFrom` / `groupAllowFrom` は両アカウント共通の safety boundary
  - **エージェント**: `agents.list[]` に `official-agent` (`anthropic/claude-opus-4-7` primary + `anthropic/claude-sonnet-4-6` fallback) と `personal-agent` (`ollama/qwen3.6:35b-a3b`)
...
デフォルトは official-agent = `anthropic/claude-opus-4-7` (+ `anthropic/claude-sonnet-4-6` fallback)、personal-agent = `ollama/qwen3.6:35b-a3b`。
...
"agents": {
  "list": [
    ...
    { "id": "personal-agent", "model": "ollama/qwen3.6:35b-a3b" }
  ]
}
```

`model` は文字列 (`"provider/model"`) または `{ primary, fallbacks }` オブジェクトのどちらでも可。`openclaw models set` は `agents.defaults.model` を書き換える CLI のため、本構成（agent 別モデル）では openclaw.json の直接編集を推奨。

### Ollama（ローカル LLM）モデルの差し替え例

Mac Mini 上の Ollama (`http://127.0.0.1:11434`) は `OLLAMA_API_KEY="ollama-local"` がセット済み（`03-openclaw-setup.sh` で `~/.openclaw/.env` に自動投入）。`personal-agent` を別の ollama モデルに切り替えるには:

```bash
# personal-agent.model を ollama/<別モデル> に書き換え
python3 -c '
import json, pathlib
p = pathlib.Path.home() / ".openclaw" / "openclaw.json"
c = json.loads(p.read_text())
for a in c["agents"]["list"]:
    if a["id"] == "personal-agent":
        a["model"] = "ollama/<新しいモデル>"
...
引数なしの場合は `~/.openclaw-snapshot-*` から最新を自動選択。古い snapshot や別パスから復元したい場合はディレクトリパスを引数で渡します（`--recover=DIR` 形式も可）。
