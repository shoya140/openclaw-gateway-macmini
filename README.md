...
- Ollama 本体は `01-admin-macos-setup.sh` が `brew install ollama` + `/Library/LaunchDaemons/io.shoya.ollama.plist` (UserName=claw) で常駐させ、モデル本体は `02-claw-user-setup.sh` が `ollama pull` する。既定モデルは `qwen3.6:35b-a3b`、`.env` の `OLLAMA_MODEL` で変更可
...
何度もセットアップを再実行する場合、プロジェクトルート（このリポジトリの直下）に `.env` を置いておくと、`03-openclaw-setup.sh` が読み込んで対話入力を省略します。`.env` は `.gitignore` 済みでコミットされません。
...
OLLAMA_MODEL="qwen3.6:35b-a3b"  # 省略可。02 の preload + 03 の personal-agent.model に反映
```
...
- Ollama インストール + `/Library/LaunchDaemons/io.shoya.ollama.plist` 配置
  - **`UserName=claw`** で起動（root ではない。最小権限）。モデル置き場は `/Users/claw/.ollama/`
  - 環境変数: `OLLAMA_FLASH_ATTENTION=1`、`OLLAMA_KV_CACHE_TYPE=q8_0`（Apple Silicon の prefill 加速 + KV キャッシュ q8 量子化）
...
- Ollama daemon の起動を待って `ollama pull "${OLLAMA_MODEL:-qwen3.6:35b-a3b}"` を実行（既に最新ならすぐ完了）
- Ollama preload LaunchAgent (`~/Library/LaunchAgents/io.shoya.ollama-preload.plist`) を配置。起動時に同モデルを `keep_alive=-1` でメモリに常駐させる
...
Mac Mini 上の Ollama (`http://127.0.0.1:11434`) は `OLLAMA_API_KEY="ollama-local"` がセット済み（`03-openclaw-setup.sh` で `~/.openclaw/.env` に自動投入）。**`personal-agent` のモデルと preload 対象モデルは `<repo>/.env` の `OLLAMA_MODEL` で一括指定**する：

```bash
# 1. <repo>/.env の OLLAMA_MODEL を書き換え
#    OLLAMA_MODEL="<新しいモデル>"

# 2. 反映 (02 が pull + preload Agent 更新、03 が openclaw.json 更新)
./scripts/02-claw-user-setup.sh
./scripts/03-openclaw-setup.sh
```

スクリプトを再実行せず手動で反映したい場合は、`ollama pull <model>` 後に `~/.openclaw/openclaw.json` の `agents.list[].model` を直接編集して `openclaw gateway restart`。
