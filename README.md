...
`exec.security: full` + `tools.deny: []` + `execApprovals: off` で「ユーザー権限内のあらゆる操作を承認なしに自走」する設計。`gateway` / `cron` / `sessions_spawn` ツールも開放されているため、エージェント自身が `openclaw.json` を書き換えたり、永続化スケジュールを作ったり、サブエージェントを spawn できる。`claw` 専用標準アカウントによる OS 分離（sudo/brew 不可、admin・他ユーザーのファイル不可視）が最後の砦。**ただし `personal-agent` は LM Studio 上の小規模ローカル LLM を使うため、prompt injection 耐性が低い。`agents.list[].sandbox.mode: "all"` + `agents.list[].tools.deny: ["group:web", "browser"]` を per-agent 適用して攻撃面を縮小する**（official-agent (Claude Opus 4.7) は web/browser/sandbox-off のまま）。
