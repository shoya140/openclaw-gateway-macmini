...
6. **LLM APIキー設定** - `openclaw models auth paste-token` でキー設定

7. **ファイルパーミッション設定** (`~/.openclaw`: 700, 設定ファイル: 600)

8. **Gatewayデーモン登録・起動・セキュリティ監査**
   ```bash
...
| `tools.fs.allowedRoots` | `["/Users/claw"]` |
...
| `gateway.port: 18789` | [ ] |
| `gateway.bind: "loopback"` | [ ] |
| Gateway認証トークン設定済み (`gateway.auth.token`) | [ ] |
...
| `tools.exec.security: "full"` | [ ] |
| ワークスペースに `AGENTS.md` (sudo/brew依頼ルール) 配置済み | [ ] |
