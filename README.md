...
Mac MiniにOpenClawを最もセキュアにインストールし、Telegramから利用するためのセットアップスクリプト + 運用マニュアル。
...
| パス | 内容 | アクセス |
|------|------|---------|
| `/Users/claw/.openclaw/` | 設定、認証、セッション等 | claw のみ（管理者は sudo） |
| `~/Library/CloudStorage/GoogleDrive-.../My Drive/openclaw-workspace/` | エージェントの作業ディレクトリ | claw（Google Drive経由で個人PCと共有） |
...
3. **ワークスペースパス検出** - Google Drive内の `openclaw-workspace` フォルダを自動検出
